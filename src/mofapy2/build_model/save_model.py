...
class saveModel:
                    ...
                    if ctype in ("object", "str"):
                        try:
                            ...
                            group_meta.create_dataset(
                                col,
                                data=np.array(self.samples_metadata[g][col], dtype="|S"),
                            )
                        ...
                        group_meta.create_dataset(
                            col,
                            data=np.array(self.samples_metadata[g][col], dtype=ctype.type),
                        )
    ...
    def saveData(self):
        ...
        if self.sample_cov is not None:
            ...
            for g in range(len(self.groups_names)):
                ...
                if sample_cov_transformed is not None:
    ...
    def saveModelOptions(self):
        ...
        opts = {k: np.asarray(self.model_opts[k]).astype("S") for k in options_to_save}
    ...
    def saveTrainOptions(self):
        ...
        self.hdf5.create_dataset("training_opts", data=np.array(list(opts.values()), dtype=float))
    ...
    def saveSmoothOptions(self, smooth_opts):
        ...
        opts = {k: np.asarray(smooth_opts[k]).astype("S") for k in options_to_save}
