...
from mofapy2.core.gp_utils import set_inducing_points
    ...
    def saver(self, *args, **kwargs):
        ...
        # Internal methods will raise TypeError when interrupted
            ...
            if self.train_opts["save_interrupted"]:
                console.print("Attempting to save the model at the current iteration...")
                ...
                logger.exception(
                    ...
                    "set save_interrupted in the training options to true."
                )
            sys.exit()
...
class entry_point:
    ...
    def set_covariates(self, sample_cov: npt.ArrayLike | str, covariates_names: str | list[str] | None = None) -> None:
        ...
        Parameters
        ----------
        sample_cov : list[npt.ArrayLike] | str
            This can be either
        ...
        covariates_names : str, list[str], optional
            String or list of strings containing the name(s) of the covariate(s) (optional)
        """

        if self.data is None:
            msg = "the data must be specified with set_data before specifying covariates"
        ...
        if not isinstance(sample_cov, list):
            match sample_cov:
                ...
                case np.ndarray():
                    sample_cov = [sample_cov]
                case _:
                    msg = "sample_cov not recognised"
                    raise ValueError(msg)

        if len(sample_cov) != self.dimensionalities["G"]:
            msg = f"sample_cov needs to be a list of same length as data (same number of groups). Got {len(sample_cov)} groups, expected {self.dimensionalities['G']}"
            raise ValueError(msg)
        ...
        if sample_cov is not None:
            console.print(f"Loaded {sample_cov.shape[1]} covariate(s) for each sample...")
    ...
    def set_data_matrix(
        ...
        if views_names is None:
            console.print("View names not provided, using default naming convention:")
        ...
        for m in range(len(data)):
            ...
            if not isinstance(data[m], np.ndarray):
    ...
    def set_data_df(self, data, likelihoods=None):
        ...
        for g in self.data_opts["groups_names"]:
            for m in self.data_opts["views_names"]:
                try:
                    console.print(
                        f"Loaded group='{g!s}' view='{m!s}' with N={tmp_samples[g][m]:0.2f} samples and D={tmp_features[g][m]:0.2f} features..."
                        % (g, m, tmp_samples[g][m], tmp_features[g][m])
                    )
                except:
                    console.print(f"No data found for group='{g!s}' and view='{m!s}'...")
        console.print("\n")
    ...
    def set_data_from_anndata(
        ...
        likelihoods: optional
            likelihoods to use (guessed from the data if not provided)
        features_subset : str, optional
            .var column with a boolean value to select genes (e.g. "highly_variable")
        save_metadata : bool
        """
    ...
    def set_train_options(
        ...
        if gpu_mode:
            try:
                ...
                console.print("[green]GPU mode[/] is [red]activated[/]")
                if gpu_device is not None:
                    cp.cuda.Device(gpu_device).use()
                    self.train_opts["gpu_device"] = gpu_device
                    console.print(f"Using GPU device {gpu_device}")
            except ImportError:
                console.print(
                    ...
                    "2 - Install CuPy following the [link=https://docs-cupy.chainer.org/en/stable/install.html]instructions[/link]",
                    sep="\n",
                    end="\n\n",
                )
                sys.stdout.flush()
        ...
        dropR2 = None if dropR2 is False else dropR2
        ...
        if verbose:
            console.print(f"Convergence mode: {convergence_mode!s}")
        ...
        if schedule is None:
            ...
            if not {"Y", "W", "Z", "Tau"} <= {schedule}:
                msg = "The schedule must contain at least the following elements: Y, W, Z, Tau"
                raise ValueError(msg)
            if self.model_opts["ard_factors"]:
                if "AlphaZ" not in schedule:
            ...
            if self.model_opts["ard_weights"]:
                if "AlphaW" not in schedule:
                    ...
                    raise ValueError(msg)
            if self.model_opts["spikeslab_factors"]:
                if "ThetaZ" not in schedule:
                    msg = "The schedule must contain 'ThetaZ' if Spike and Slab prior on Z is activated"
                    raise ValueError(msg)
            if self.model_opts["spikeslab_weights"]:
                if "ThetaW" not in schedule:
                    msg = "The schedule must contain 'ThetaW' if Spike and Slab prior on W is activated"
                    raise ValueError(msg)
    ...
    def set_smooth_options(
        ...
        if self.smooth_opts["warping"] is True:
            if self.dimensionalities["G"] <= 1:
                msg = "The warping functionality is only relevant when having multi-group data"
                raise ValueError(msg)
            if self.dimensionalities["C"] != 1:
                msg = "Warping only implemented for one dimensional covariates"
                raise ValueError(msg)
            console.print("Warping set to [green]True[/green]: aligning the covariates across groups")
        ...
        if sparseGP is True:
            if self.smooth_opts["warping"]:
                msg = "The warping functionality cannot be used in conjunction with the sparseGP option."
                raise ValueError(msg)
            if not all(l == "gaussian" for l in self.model_opts["likelihoods"]):
                msg = "Non-gaussian likelihoods cannot be used in conjunction with the sparseGP option."
                raise ValueError(msg)
            self.smooth_opts["sparseGP"] = True
            if self.dimensionalities["N"] >= 1000:
                console.print(
                    "[blink red]Warning[/]: sparseGP should only be used when having a large sample size (>1e3)"
                )
    ...
    def set_model_options(
        ...
        if (self.dimensionalities["G"] > 1) & (not ard_factors):
            console.print(
                "[blink red]WARNING[/blink red]: 'ard_factors' in model_options should be set to True if using multiple groups unless you are using [green]MEFISTO[/green]"
            )
        ...
        if (self.dimensionalities["M"] > 1) & (not ard_weights):
            console.print(
                "[blink red]WARNING[/blink red]: 'ard_weights' in model_options should be set to [blue]True[/blue] if using multiple views"
            )
    ...
    def build(self):
        ...
        if not hasattr(self, "smooth_opts"):
            if any(counts < minimum_number_of_samples_to_be_useful):
                console.print(
                    f"Warning: some group(s) have less than {minimum_number_of_samples_to_be_useful} samples, MOFA won't be able to learn meaningful factors for these group(s)..."
                )
    ...
    def predict_factor(
        ...
        Parameters
        ----------
        new_covariates : str | list[str], optional
            new covariate values to predict at, should be of same format as covariates.
            If None all present covariate values are used
        uncertainty : bool, default True
            provide uncertainty measures for the prediction
        groups : str, default="all"
            Groups for which to predict
        """
...
def mofa(
    ...
    use_layer: str | None = None,
    features_subset: str | None = None,
    likelihood: str | list[str] | None = None,
    n_factors: int = 10,
    ...
    """Helper function to init and build the model in a single call from annotation data object
    ...
    likelihood : str | list[str], optional
        likelihood to use, default is guessed from the data
    ...
    convergence_mode : Literal["fast", "medium", "slow"], default="fast":
    ...
    expectations : listr[str], optional:
    ...
    verbose : bool, default=False:
        print verbose information during training
